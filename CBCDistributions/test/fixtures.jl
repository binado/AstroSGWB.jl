...
propagation_hyperprior(::Type{GR}) = product_distribution(NamedTuple())
function propagation_hyperprior(::Type{<:ModifiedPropagation})
    return product_distribution((Ξ₀ = Uniform(0.5, 5.0), Ξₙ = Uniform(0.05, 3.0)))
end

function merge_hyperpriors(cosmo_hp, prop_hp, pop_hp)
    return product_distribution(merge(cosmo_hp.dists, prop_hp.dists, pop_hp.dists))
end
