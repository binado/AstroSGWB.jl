...
# Population with a custom per-component `logpdfdiffvec` override: the `:y` factor is
...
function CBCDistributions.logpdfdiffvec(
        ::SkipYPop,
        ::Val{:y},
        d_target,
        d_proposal,
...
function cosmology_hyperprior(::Type{<:ModifiedPropagation{C}}) where {C <:
                                                                       AbstractCosmology}
    return product_distribution(merge(
