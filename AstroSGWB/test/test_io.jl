...
@testset "prepared model reconstructs derived caches" begin
    ...
    fs = fiducial_spectral_density(model, problem)
...
@testset "fiducial spectral density differs across cosmologies" begin
    ...
    fs_w0 = fiducial_spectral_density(loaded_w0.model, loaded_w0.problem)
    ...
    prepared_lcdm = prepare_parity_model(
        p_lcdm, C_lcdm, P, grid, _TEST_LOAD_DETS, 1.0, 161.0)
    ...
    @test !(fs_w0 ≈ fiducial_spectral_density(model_lcdm, p_lcdm))
end
