...
@testset "AdvancedHMC smoke test" begin
    ...
    h5open(fixture_path, "r") do file
        for (cache_filename, group_name) in (
            ...
            logdensity, gradient = LogDensityProblems.logdensity_and_gradient(ad_problem, z0)
            reference_logdensity, reference_gradient =
                finite_difference_logdensity_and_gradient(problem, z0)
